...
`AutoInstall` marks an addon the background pass installs when it is missing, under the same admin gate as auto-update; it cannot be hidden. `GitHubRepo` sources releases from the GitHub API instead of the manifest host (`GitHubReleases` in Core): one `GET /releases?per_page=20` per repo, cached for 15 minutes in the process, from which `release` is the newest non-draft entry without the `prerelease` flag and `pre-release` the newest with it; there is no `development` channel for these addons. The `.zip` asset is the release and its `sha256:` digest is the checksum, and the tag name is the version (RXPGuides stamps the same `v4.11.4` form into its TOC). The request carries a `User-Agent` because GitHub answers 403 without one. Unauthenticated calls are limited to 60 per hour per IP and a 403 "rate limit exceeded" surfaces in the error bar, so the budget is one call per GitHub repo (addons and the app's own update check alike) per 15 minutes at most, and the minute-by-minute retry while the guild API is unreachable re-checks only the guild API. The icon is the repo owner's avatar, `https://github.com/{owner}.png?size=64`; a manifest-host addon serves its own `icon.png` beside the manifests.
