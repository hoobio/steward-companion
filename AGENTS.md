...
Three release channels: `stable`, `beta`, `unstable`. The channel is per addon, stored in `Channels` alongside the install-state records in the same `state.json`, defaulting to the first channel that has a release in the order `beta`, `stable`, `unstable` (`AddonChannelStatus.DefaultPreference`; the picker itself lists `stable`, `beta`, `unstable`). Beta leads while the addons are pre-release; the order moves to stable first once a stable channel is the norm. `unstable` is offered only when `/api/admin/me` returns `role: "global"` exactly; `admin` sees stable and beta. That gate is client-side only, same caveat as the admin re-check above: nothing stops a direct fetch of the unstable manifest.
