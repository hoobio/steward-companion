...
- **No field of `ClubMemberInfo` is flagged secret, but `C_Club.GetClubMembers` (`ClubDocumentation.lua:435`) and `C_Club.GetMemberInfo` (`:613`) both are.** Reading the struct is not enough to be safe: under chat messaging lockdown the `name`, `memberNote` and `officerNote` that come back are secret strings, so they are guarded with `canaccessvalue` and the member is skipped when the guard fails, and they are never stored. Checking the struct alone and concluding the path was safe is a mistake this contract already made once.
