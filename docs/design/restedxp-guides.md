...
Consequences: several products per install; no client-closed rule, since the file is only read at login or `/reload`, so the row reads "Imports on next login or /reload"; the BattleTag check still runs inside the importer's integrity check, which derives its key from `BNGetInfo`, so Battle.net must be connected at the 3-second mark. The addon imports every string on every login, a few seconds of work: a `StewardGuidesDB` saved variable records the hash of each string that finished with "Guides Loaded Successfully", but only so Steward can confirm the import. It is never used to skip one, because on this client our small saved variable loads while RXPGuides' own guide cache does not (forever-bugs#34), and a skip on our mark left the game with no guides on 20 Sep 2026.
